...
def updateStudentRedirect(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect('update_student_')
...
def takeAttendence(request):
    if request.method == 'POST':
        ...
        if Attendence.objects.filter(date = str(date.today()),branch = details['branch'], year = details['year'], section = details['section'],period = details['period']).count() != 0 :
            ...
            return render(request, 'attendence_sys/takeattendance.html')
        else:
    ...
    context = {}
    return render(request, 'attendence_sys/takeattendance.html', context)#we have to change
...
def home2Page(request):
    return render(request,'attendence_sys/home2.html')

def update_student_(request):
    return render(request,'attendence_sys/update_student_.html')
